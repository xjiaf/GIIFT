...
class MultiModalDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_src_caption = params.use_src_caption
        self.src_lang = params.src_lang
        self.tgt_lang = params.tgt_lang

        if self.is_pretraining:
            self.clip_embs = clip_embs[self.src_lang] if isinstance(
                clip_embs, dict) else clip_embs
            self.clip_tok = clip_tok_data[self.src_lang]

            if self.use_src_caption:
                # outputs are the same as inputs (self-supervised)
                self.input_tok = tok_data[self.src_lang]
                self.output_tok = tok_data[self.src_lang]
                self.output_raw = raw_data[self.src_lang]
                self.caption_lang = self.src_lang
            else:
                # outputs are the target language tokenized data (translation)
                self.input_tok = tok_data[self.src_lang]
                self.output_tok = tok_data[self.tgt_lang]
                self.output_raw = raw_data[self.tgt_lang]
                self.caption_lang = self.tgt_lang

            self.length = len(self.input_tok)
        else:
            self.input_tok = tok_data[self.src_lang]
            self.output_tok = tok_data[self.tgt_lang]
            self.output_raw = raw_data[self.tgt_lang]
    ...
    def mask_inputs_(self, inputs):
        if self.mask_inputs:
            mask_inputs = torch.rand(
                inputs['input_ids'].shape) >= self.mask_prob
            mask_inputs[0][0] = mask_inputs[0][-1] = True  # Keep LID and EOS
            return {
                'input_ids': inputs['input_ids'][mask_inputs],
                'attention_mask': inputs['attention_mask'][mask_inputs]
            }, mask_inputs
        return inputs, None
    ...
    def __getitem__(self, idx):
        ...
        if self.is_pretraining:
            ...
            inputs, mask = self.mask_inputs_(self.input_tok[idx])
            batch.append(inputs)
            batch.append(mask)
            ...
            batch.append(inputs)
            outputs = self.output_tok[idx]
            mask = torch.ones_like(outputs['input_ids']).bool()
            batch.append(mask)
            batch.append(outputs)
            batch.append(self.output_raw[idx])
